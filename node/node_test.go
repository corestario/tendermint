...
func TestNodeStartStop(t *testing.T) {
	...
	blocksSub, err := n.GetEventBus().Subscribe(context.Background(), "node_test", types.EventQueryNewBlock)
	require.NoError(t, err)
	select {
	case <-blocksSub.Out():
...
func TestNodeDelayedStart(t *testing.T) {
	...
	// create & start node
	n, err := DefaultNewNode(config, log.TestingLogger())
	n.GetGenesisDoc().GenesisTime = now.Add(2 * time.Second)
	require.NoError(t, err)
	...
	assert.Equal(t, true, startTime.After(n.GetGenesisDoc().GenesisTime))
}
...
func TestNodeSetPrivValTCP(t *testing.T) {
	...
	n, err := DefaultNewNode(config, log.TestingLogger())
	require.NoError(t, err)
	assert.IsType(t, &privval.SignerClient{}, n.GetPrivValidator())
}
