...
func verifyDKGEvidence(dkgInstance dkgTypes.DKG, evidence types.Evidence) error {
	...
	case *DKGEvidenceMissingData:
		if !dkgInstance.CheckLoserMissingData(v.Loser) {
			return errors.New("CheckLoserMissingData confirmed missing DKG data")
		}
	case *DKGEvidenceCorruptData:
		if !dkgInstance.CheckLoserCorruptData(v.Loser) {
			return errors.New("CheckLoserCorruptData confirmed corrupt DKG data")
		}
	case *DKGEvidenceCorruptJustification:
		if !dkgInstance.CheckLoserCorruptJustification(v.Loser) {
			return errors.New("CheckLoserCorruptJustification confirmed corrupt DKG Justification")
		}
	}
