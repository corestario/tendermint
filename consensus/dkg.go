...
func (cs *ConsensusState) handleDKGShare(mi msgInfo) {
	...
	if !ok {
		...
		dealer = NewDKGDealer(cs.Validators, cs.privValidator, cs.sendDKGMessage, cs.Logger, msg.RoundID)
...
func (cs *ConsensusState) startDKGRound() error {
	...
	if !ok {
		...
		dealer = NewDKGDealer(cs.Validators, cs.privValidator, cs.sendDKGMessage, cs.Logger, cs.dkgRoundID)
...
func NewDKGDealer(validators *types.ValidatorSet, privValidator types.PrivValidator, sendMsgCb func(*types.DKGData), logger log.Logger, startRound int) *DKGDealer {
	return &DKGDealer{
