...
func TestByzantineDKG(t *testing.T) {
	...
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, nil, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontSendOneDeal(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerNoDeal})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontAnyDeals(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerAnyDeal})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontSendOneResponse(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerNoResponse})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontAnyResponses(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerAnyResponses})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontSendOneJustification(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerNoJustification})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontAnyJustifications(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerAnyJustifications})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontSendOneCommit(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerNoCommit})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestByzantineDKGDontAnyCommits(t *testing.T) {
	...
	T := 3
	logger := consensusLogger().With("test", "byzantine")
	dkgConstructor := dealer.NewDealerConstructor(map[int]dealer.DKGDealerConstructor{0: dealer.NewDKGMockDealerAnyCommits})
	css, cleanup := randConsensusNet(N, "consensus_byzantine_test", newMockTickerFunc(false), newCounter, dkgConstructor, offChain.GetVerifier(T, N), dkgBlock)
	defer cleanup()
...
func TestDKGVerifyMessage(t *testing.T) {
	...
	dealer := dealer.NewDKGDealer(validators, privVal, nil, nil, consensusLogger().With("test", "byzantine"), 0)
	testAddr := []byte("some_test_address")
	testData := []byte("some_test_data")
